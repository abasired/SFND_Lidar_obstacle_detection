...
clusterHelper(int index, const std::vector<std::vector<float>> points, std::vector<int>& cluster,std::vector<bool> processed, KdTree* tree, float distanceTol){
  ...
  for(int id : nearest){
    if(!processed[id]){
      clusterHelper(id, points, cluster, processes, tree, distanceTol);
    }
  }
...
int main ()
{
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	std::vector<int> nearby = tree->search({7.3,7},3.0);
